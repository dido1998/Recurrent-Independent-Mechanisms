...
class SparseRIMCell(nn.Module):
    def __init__(self, 
        device, input_size, hidden_size, num_units, k, rnn_cell, input_key_size = 64, input_value_size = 400, input_query_size = 64,
        num_input_heads = 1, input_dropout = 0.1, comm_key_size = 32, comm_value_size = 100, comm_query_size = 32, num_comm_heads = 4, comm_dropout = 0.1,
        a = 1, b = 3, threshold = None # for beta dist
    ):
        super().__init__()
        if comm_value_size != hidden_size:
            #print('INFO: Changing communication value size to match hidden_size')
        ...
        self.device = device
        self.hidden_size = hidden_size
        self.num_units =num_units # capital K
        self.rnn_cell = rnn_cell
        self.key_size = input_key_size

        self.k = k
        self.threshold = 0.5 if threshold is None else threshold # correct syntax?
        ...
        self.eta = self.eta_0
        self.nu = nn.parameter.Parameter(data=torch.ones(self.num_units)*self.nu_0,
                                        requires_grad=False)
    ...
    def input_attention_mask(self, x, h):
        ...
        self.eta = self.eta_0 + x.shape[0] # eta_0 + N
        alpha = (self.eta-self.nu+1)/(self.eta+2) # expectation of alpha
        ...
        attention_probs = nn.Softmax(dim = -1)(attention_scores)
        # attention_probs[:,:,0:-1] = attention_probs[:,:,0:-1]*self.alpha.view(1,-1,1)
        attention_probs[:,:,-1] = 1 - torch.sum(attention_probs[:,:,0:-1], dim=2)

        # new mask
        not_null_probs = 1 - attention_probs[:,:,-1]
        mask_ = torch.ceil(not_null_probs-self.threshold)


        attention_probs = self.input_dropout(attention_probs) # do we still need it? 
        inputs = torch.matmul(attention_probs, value_layer) * mask_.unsqueeze(2)
...
class SparseRIMCell2(nn.Module):
    def __init__(self, 
        device, input_size, hidden_size, num_units, k, rnn_cell, input_key_size = 64, input_value_size = 400, input_query_size = 64,
        num_input_heads = 1, input_dropout = 0.1, comm_key_size = 32, comm_value_size = 100, comm_query_size = 32, num_comm_heads = 4, comm_dropout = 0.1,
