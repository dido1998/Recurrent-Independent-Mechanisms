...
class ACModel(nn.Module, torch_ac.RecurrentACModel):
    def __init__(self, obs_space, action_space, use_memory=True, use_text=False, use_rim = False):
        ...
        if self.use_memory:
            if use_rim:
                self.memory_rnn = RIMCell(torch.device('cuda'), self.image_embedding_size, self.semi_memory_size // NUM_UNITS, NUM_UNITS, 'LSTM', input_value_size = 64, comm_value_size = self.semi_memory_size // NUM_UNITS, k = 3 )
            else:
